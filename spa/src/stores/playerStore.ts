...
export const usePlayerStore = defineStore('player', {
  ...
  actions: {
    ...
    leaveChannel(topicName:string, innerName: string|null = null) {
      ...
      if (!topicName.startsWith('listeners:')) {
        Object.entries(this.song).forEach(
          ([key, value]) => {
            if ((value as SongContainer).topic === topicName) {
    ...
    setTimer(minutes: number|null) {
      ...
      if (minutes === null || minutes === 0) {
        global.displayToast({
          message: i18n.global.t('message.player.timer.cancelled'),
      ...
      if (minutes !== null && minutes > 0) {
        global.displayToast({
          message: i18n.global.t('message.player.timer.set', { minutes }),
          type: config.TOAST_TYPE_INFO
        });
        ...
        this.timerInterval = setInterval(() => {
          ...
          if (this.timer === 0) {
            ...
            global.displayToast({
              message: i18n.global.t('message.player.timer.finish'),
              type: config.TOAST_TYPE_INFO
            });
          }
        }, 60000);
