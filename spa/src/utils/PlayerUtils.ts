...
// generated by Claude.ai 3.5

const getYoutubeId = (url: string): [VideoProvider, string]|null => {
  const patterns = [
    /(?:https?:\/\/)?(?:www\.)?youtube\.com\/watch\?v=([^&]+)/,
  ...
  for (const pattern of patterns) {
    ...
    if (match && match[1]) {
      return [VideoProvider.Youtube, match[1]];
    }
  }
...
const getDailymotionId = (url: string): [VideoProvider, string]|null => {
  const patterns = [
    /(?:https?:\/\/)?(?:www\.)?dailymotion\.com\/video\/([^?]+)/,
  ];
  ...
  for (const pattern of patterns) {
    const match = url.match(pattern);
    if (match && match[1]) {
      return [VideoProvider.Dailymotion, match[1]];
    }
  }
...
const getVideoId = (url: string): [VideoProvider, string]|null => {
