...
[Programmes-Radio](https://www.programmes-radio.com/en/) is a website displaying French radios schedule and a stream player. This article will explore how we wrote the "currently playing song" feature for the player.
...
We start with a basic Phoenix project.
...
2. We add a refresh timer to our GenServer that monitor the Presence tracker and terminates the process if no clients are connected (notice that we don't have to write any code to manage clients disconnection).
