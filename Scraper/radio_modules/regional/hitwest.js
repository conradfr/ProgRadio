...
const format = async dateObj => {
  ...
  const cleanedData = await daySchedule.reduce(async function (prevP, curr) {
    ...
    let descriptionSupp = await fetchDesc(`https://www.hitwest.com${curr.slug}`);

    if (typeof descriptionSupp === 'object' && descriptionSupp.description.length > 0) {
      description += ' ' + descriptionSupp.description.join(' ');
    }
...
const fetch = dateObj => {
  let url = 'https://www.hitwest.com/emissions';
