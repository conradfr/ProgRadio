  ...
  defp base_query() do
    from s in Stream,
      ...
      select: %{
        ...
        img_alt: fragment("COALESCE(CASE WHEN bool_or(?) is TRUE THEN ? ELSE null END, ?)", rs.own_logo, rs.code_name, r.code_name),
        current_song:
          fragment(
        ...
        radio_stream_code_name:
          fragment(
            "CASE WHEN(? IS NOT NULL and ? = TRUE) THEN COALESCE(?, CONCAT(?, '_', ?)) ELSE ? END",
            ss.code_name,
            ss.enabled,
