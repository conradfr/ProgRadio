defmodule ProgRadioApi.AutoUpdater.RadioStreams.RadioStreamTask do
  ...
  def start_link(radio_stream) do
    ...
    case get_updated_url(radio_stream) do
      ...
      {:ok, new_url} when is_binary(new_url) ->
        Logger.debug("Auto-update stream: url has not changed- #{radio_stream.code_name}", [code_name: radio_stream.code_name])
        update_in_db(radio_stream, new_url)
      {_, nil} ->
        Logger.debug("Auto-update stream: nothing returned- #{radio_stream.code_name}", [code_name: radio_stream.code_name])
        update_in_db(radio_stream, nil)
    end
  end

  defp get_updated_url(%RadioStream{} = radio_stream) when radio_stream.radio_stream_update.type == "json" do
    try do
      path = String.split(radio_stream.radio_stream_update.path, ".")
      url = Req.get!(radio_stream.radio_stream_update.url)
      |> Map.get(:body)
      |> Kernel.get_in(path)
  ...
  defp update_in_db(%RadioStream{} = radio_stream, new_stream_url) when is_binary(new_stream_url) do
