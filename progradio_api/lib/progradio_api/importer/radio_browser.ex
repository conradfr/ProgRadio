defmodule ProgRadioApi.Importer.StreamsImporter.RadioBrowser do
  ...
  # todo refactor if more formats
  defp stream_url_transformer(stream_url) do
    # Matches URLs from streamtheworld with any number after the colon
    pattern_streamtheworld = ~r/(https?:\/\/\d+\.live\.streamtheworld\.com(:\d+)?\/)(\w+)/

    # Check if the stream_url matches the pattern
    case Regex.match?(pattern_streamtheworld, stream_url) and
      ...
      true ->
        ...
        Regex.replace(pattern_streamtheworld, stream_url, replacement_fn)
