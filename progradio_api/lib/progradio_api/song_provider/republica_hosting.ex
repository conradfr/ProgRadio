defmodule ProgRadioApi.SongProvider.RepublicaHosting do
  require Logger
  alias ProgRadioApi.SongProvider

  # copied from asura_hosting.ex
  # todo mutualize?
  ...
  def get_data(name, _last_data) do
    Logger.debug("Data provider - #{name} (republicahosting): data")
  ...
  def get_song(name, data, _last_song) do
    try do
      Logger.debug("Data provider - #{name} (republicahosting): song")
      ...
      _ ->
        Logger.debug("Data provider - #{name} (republicahosting): song error rescue")
        :error
    end
  end

  defp get_shoutcast_data(url) do
    Logger.debug("Data provider - #{url} (republicahosting shoutcast): data")
  ...
  defp get_icecast_data(url) do
    Logger.debug("Data provider - #{url} (republicahosting icecast): data")
